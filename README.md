...
All backend contact goes through two calls in `js/api.js`; everything else is UI. To connect the
real site:

1. In `js/config.js` set `mode: 'stuntlisting'`.
...
Endpoint paths are configurable in `js/config.js` if you'd rather mount them elsewhere.

## Customizing

Everything below lives in `js/config.js` / CSS variables:
...
- **Look & feel** — colors are CSS variables at the top of `css/styles.css`.
...
| File | What it is |
| --- | --- |
| `index.html` | The page: photo grid, details form, coordinator box, adjust dialog |
| `css/styles.css` | All styling (dark theme) |
| `js/config.js` | Mode, endpoint paths, output size/format |
| `js/api.js` | Session + performer search adapter (demo and live implementations) |
| `js/outlines.js` | The dashed passport-style head guides (front + profile SVG paths) |
| `js/composer.js` | Canvas rendering: shared photo-framing math and final-sheet composition |
| `js/app.js` | UI wiring: uploads, drag/swap, adjust dialog, autocomplete, create/download |
